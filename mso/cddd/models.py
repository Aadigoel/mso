...
class BaseModel(ABC):
    ...
    def __init__(self, mode, iterator, hparams):
        ...
        self.embedding_size = hparams.emb_size # 512
        ...
        self.char_embedding_size = hparams.char_embedding_size # 32
        self.global_step = tf.get_variable('global_step',
                                           [],
                                           initializer=tf.constant_initializer(0),
                                           trainable=False)
        self.save_dir = hparams.save_dir # saves/run3_emb_512_5
        self.checkpoint_path = os.path.join(self.save_dir, 'model.ckpt')
        self.batch_size = hparams.batch_size # 64
        self.rand_input_swap = hparams.rand_input_swap # true
    ...
    def build_graph(self):
        ...
        if self.mode in ["TRAIN", "EVAL"]:
            ...
            with tf.variable_scope("Decoder"):
                ...
                # So the decoder output is logits, that's probabilities. Probabilites of what?
                self.prediction = tf.argmax(logits, axis=2, output_type=tf.int32) # What is the argmax returning here
