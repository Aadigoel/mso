...
class InputPipeline():
    ...
    def __init__(self, mode, hparams):
        ...
        Returns:
            None
        """
        self.mode = mode #Can be train/eval etc etc
        self.batch_size = hparams.batch_size # 64
        self.buffer_size = hparams.buffer_size # 10000
        self.input_sequence_key = hparams.input_sequence_key # random smiles in hparams.json
        self.output_sequence_key = hparams.output_sequence_key # canonical smiles in hparams.json
        if self.mode == "TRAIN":
            self.file = hparams.train_file # ../data/pretrain_dataset.tfrecords
        else:
            self.input_sequence_key = "canonical_smiles"
            self.file = hparams.val_file # ../data/pretrain_dataset_val.tfrecords but where are these??
        self.encode_vocabulary = {
            v: k for k, v in np.load(hparams.encode_vocabulary_file, allow_pickle=True).item().items()
        } # Where is the encode vocab file??
        self.decode_vocabulary = {
            v: k for k, v in np.load(hparams.decode_vocabulary_file, allow_pickle=True).item().items()
        }
        self.num_buckets = hparams.num_buckets # 8.0
        self.min_bucket_lenght = hparams.min_bucket_length # 20.0
        self.max_bucket_lenght = hparams.max_bucket_length # 60.0
    ...
    def make_dataset_and_iterator(self):
        ...
        self.dataset = tf.data.TFRecordDataset(self.file) # Loads the dataset as bytes without parsing or decoding
        if self.mode == "TRAIN":
            self.dataset = self.dataset.repeat() # repeats an arbitrary number of times
        self.dataset = self.dataset.map(self._parse_element, num_parallel_calls=32) # decodes the dataset
...
class InputPipelineWithFeatures(InputPipeline):
    ...
    def __init__(self, mode, hparams):
        ...
        self.num_features = hparams.num_features # 7 What are these features?
